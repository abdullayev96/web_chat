from .models import MessageModel, DialogsModel, User, UploadedFile
...
def serialize_dialog_model(m: DialogsModel, user_id):
    ...
    other_user_pk, other_user_username = User.objects.filter(pk=m.user1.pk).values_list('pk',
                                                                                             username_field).first() \
        if m.user2.pk == user_id else User.objects.filter(pk=m.user2.pk).values_list('pk', username_field).first()

...
from django_private_chat2.models import MessageModel, DialogsModel, User, UploadedFile
...
@database_sync_to_async
def get_user_by_pk(pk: str) -> Awaitable[Optional[User]]:
...
@database_sync_to_async
def save_text_message(text: str, from_: User, to: User) -> Awaitable[MessageModel]:
    return MessageModel.objects.create(text=text, sender=from_, recipient=to)


@database_sync_to_async
def save_file_message(file: UploadedFile, from_: User, to: User) -> Awaitable[MessageModel]:
